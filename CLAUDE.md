...
### 문서별 역할 고정 (STEP NEXT-AE-FIX)

**시스템 레이어별 문서 역할 (Hard Rule)**:

| 문서 | 시스템 내 역할 | 사용 레이어 |
|------|--------------|------------|
| **가입설계서 - 요약표** | 담보명 / 가입금액 / 보험료 / 가입형태 | Comparison (비교 대상) |
| **가입설계서 - 상세표** | 비교 설명용 보장 개요 (Comparison Description) | Comparison (설명) |
| **사업방법서** | 지급조건 / 구조적 제한 / 가입 가능 조건 | Evidence (근거) |
| **상품요약서** | 고객 친화 요약 (보조 설명) | Evidence (근거) |
| **약관** | 정의 / 지급 / 면책 / 감액의 법적 근거 | Evidence (근거) |

#### 1. 가입설계서 (Proposal)
- **요약표 (Summary)**: 비교 대상(coverage universe)의 유일한 기준 (SSOT)
  - 담보명, 가입금액, 보험료, 가입형태
  - Universe Lock의 기준 문서
  - **역할:** 무엇을 비교할 것인가 (What to compare)
- **상세표 (Detail)**: 비교 설명용 보장 개요
  - 회사별 담보 설명 비교에 사용
  - **역할:** 어떻게 설계되어 있는가 (How it's designed)
  - ❌ Evidence로 사용 금지 (근거가 아닌 비교 대상의 외형)
...
### Evidence의 정의 (헌법 수준 고정)

**Evidence란 다음 조건을 모두 만족해야 한다**:

1. ✅ PDF 문서 기반일 것
2. ✅ Deterministic 추출일 것 (Structure / Rule 기반, LLM 추론 금지)
3. ✅ 출처 문서가 **약관 / 사업방법서 / 상품요약서** 중 하나일 것
4. ✅ 반드시 다음 메타데이터를 포함할 것:
   - `source_doc_type` (policy / business_rules / product_summary)
   - `source_page` (페이지 번호)
   - `excerpt` (원문 발췌)

**절대 금지**:
- ❌ 가입설계서(요약/상세)에서 추출된 텍스트는 Evidence가 될 수 없다
- ❌ LLM 요약 결과를 Evidence로 저장
- ❌ page / source_doc_type 없는 Evidence
- ❌ 근거 없이 비교 결과를 단정적으로 표현

### 시스템 레이어 분리 (STEP NEXT-AE-FIX)

#### Comparison Layer (비교 계층)
**입력**:
- v2.proposal_coverage (가입설계서 요약표)
- v2.coverage_mapping (신정원 코드 매핑)

**역할**:
- 담보 존재 여부 비교
- 금액/가입조건 비교
- 가입설계서 상세표 기반 보장 설명 비교

**목적**: "회사 간 상대 비교" 전용

#### Evidence Layer (근거 계층)
**입력**:
- v2.coverage_evidence (약관/사업방법서/상품요약서 기반)
...
- ❌ 가입설계서 상세표를 Evidence로 저장 (STEP NEXT-AE-FIX)
- ❌ LLM 요약 결과를 Evidence로 저장 (STEP NEXT-AE-FIX)
- ❌ page / source_doc_type 없는 Evidence (STEP NEXT-AE-FIX)

### 문서 역할 비유

- **가입설계서 = 지도** (목적지 결정)
- **상품요약서 = 안내서** (개요 설명, Evidence)
- **사업방법서 = 운영 매뉴얼** (실무 규칙, Evidence)
- **약관 = 나침반** (방향 해석, Evidence)

**지도 없이 나침반만으로는 목적지를 정할 수 없다.**
**지도는 비교 대상을, 나침반은 근거를 제공한다.**
