...
**Commits:** 57cc9b3 (migration), 9611fce (af-fix)
**Summary:** Proposal Detail(상세표) → Comparison Description 프로덕션 품질 완성. 템플릿 격리 + 수동 데이터 제거.
**DoD:** ✅ Pass

**Migration Safety (57cc9b3):**
- ❌ DROP TABLE 제거 → ✅ CREATE IF NOT EXISTS (데이터 보존)
- ✅ extraction_method 컬럼 추가 (deterministic_v1/v2/manual 구분)
- ✅ Idempotent migration (재실행 안전)

**Extraction Quality (9611fce):**
...
- ✅ Template isolation: 동일 template_id 내에서만 매칭
- ✅ Deterministic only: extraction_method='deterministic_v2'

**ViewModel Fix:**
- ✅ Row-level matching: 각 fact_table row에 개별 comparison_description
...
**Smoke Test (Test 9):**
- ✅ Match rate 표시 (11.1%)
- ✅ Manual entry detection (WARNING if exists)
- ✅ Quality threshold (< 10% WARNING)
- ✅ DoD: >= 1 template + >= 1 matched + no manual

**Results:**
- 36 details extracted (vs 7 in AF v1)
- 4 matched (11.1% match rate)
- 0 manual entries
- ✅ Test 9 PASS

**Data Quality Issue:**
- ⚠️ proposal_coverage에 meta-header row 발견 (통합고객...)
- 현재: AF에서 meta row 필터링으로 회피
- 향후: Proposal Summary ingest 버그 픽스 별도 티켓
